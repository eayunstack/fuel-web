...
define(['utils', 'expression', 'deepModel'], function(utils, Expression) {
    ...
    models.Interface = Backbone.Model.extend({
        ...
        validate: function() {
            ...
            var maxUntaggedNetworksCount = networks.where({name: 'public'}).length && (networks.where({name: 'floating'}).length || networks.where({name: 'ceph_cluster'}).length || networks.where({name: 'vxlan_tunnel'}).length) ? 2 : 1;
